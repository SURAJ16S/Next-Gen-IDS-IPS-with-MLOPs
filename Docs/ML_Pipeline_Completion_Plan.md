...
**What is DONE:**
* **Phase A**: Redis Reputation Logic is implemented.
* **Phase E**: ML Baseline Generation (`ML/generate_baseline.py`) was run successfully, outputting `http.jsonl` and `flow.jsonl`.
* **Phase E**: Initial unsupervised anomaly models (HTTP and Flow) are trained and pushed to `ML/models/`.
* **Dataset Assessment**: Created `Security/ML Planning/dataset_assessment.md` detailing modern attack payloads (GoTestWAF, LLMs) and threat intel feeds.

**What is PENDING:**
* **Phase F (Labeled Training)**: Run sandbox to generate labeled attack traffic (GoTestWAF/LLM jailbreaks) and train the supervised Web Classifier.
* **Phase C & H (Frontend)**: Build the Admin Blocklist dashboard and integrate `<CaptchaGate>` into auth endpoints.
* **Phase G (Wiring)**: Complete FastAPI ML routing and connect the Go `http_analyzer` tier to it.
...
| A.4 | Feed ingestion goroutine (AbuseIPDB, Spamhaus, FireHOL) | [ ] | Suraj |
| E.1 | Collect baseline traffic JSONL logs (http + flow) | [x] | Suraj |
| E.2 | Train Model #1 — http-anomaly Isolation Forest | [x] | Suraj |
| E.3 | Train Model #3 — flow-anomaly Isolation Forest | [x] | Suraj |
| G.1 | Start FastAPI scoring service on port 8500 | [ ] | Suraj |
| G.2 | Create `Security/detect/ml_client.go` | [ ] | Suraj |
| G.3 | Call ML client from `http_analyzer.go` after Tier 1 | [ ] | Suraj |
| F.1 | Run sandbox + `generate_labeled_traffic.sh` | [ ] | Suraj |
| F.1a| Run GoTestWAF for evasive API payload generation| [ ] | Suraj |
| F.1b| Download/Ingest LLM Jailbreak dataset payloads  | [ ] | Suraj |
| F.2 | Train Model #2 — web-classifier Random Forest | [ ] | Suraj |
| F.3 | (Stretch) Train Model #4 — SSH brute-force | [ ] | Suraj |
| F.4 | (Stretch) Train Model #5 — DNS DGA | [ ] | Suraj |
| C.1 | Wire `<CaptchaGate>` into `Login.jsx` + `Register.jsx` | [ ] | Suraj |
| C.2 | Apply `captcha.middleware.js` on auth routes | [ ] | Suraj |
| H.1 | Create `AdminManageBlocklist.jsx` | [ ] | Suraj |
